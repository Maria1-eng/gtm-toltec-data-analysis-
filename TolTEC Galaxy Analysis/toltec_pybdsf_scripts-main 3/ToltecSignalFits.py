...
###Segmentation of extended images    
    def segment(self,npixels=5):
      ...
      #bounds = np.linspace(0, 29, 30)
      #normseg = mpl.colors.BoundaryNorm(bounds, cmap.N)

      
      #fig, (ax0,ax1,ax2) = plt.subplots(3, 1, figsize=(5, 60),subplot_kw=dict(projection=wcs))
      #fig.subplots_adjust(hspace=0.45)
      ##hdu_model=fits.open('/home/javier/propuestas/gtm/ngc3938_11mm_eqCalz_MJypersr_pixsize1_conv7.fits')
      #hdu_model=fits.open('/home/javier/propuestas/gtm/ngc3938_11mm_eqCalz_MJypersr.fits')
      #norm = simple_norm(hdu_model[0].data, 'sqrt')
      #im0=ax0.imshow(hdu_model[0].data[x0:x1,y0:y1],extent=(x0,x1,y0,y1), origin='lower', cmap='Greys_r',vmin=0,vmax=9)
      #ax0.set_title('NGC3938 input model 1100 microns')
      #ax03 = fig.add_axes([.85,0.4,0.03,0.45])
      #cbar0 = fig.colorbar(im0,orientation='vertical',cax=ax03,label='$F_{\\nu}$[MJy/sr]')
      
      ##cb0 = mpl.colorbar.ColorbarBase(ax1, norm=norm)
      #norm = simple_norm(image, 'sqrt')
      
      #im1=ax1.imshow(image[x0:x1,y0:y1]/flux_fact,extent=(x0,x1,y0,y1), origin='lower', cmap='Greys_r',vmin=0,vmax=9)
      ...
      #cb = mpl.colorbar.ColorbarBase(ax3, cmap=cmap, norm=normseg,
