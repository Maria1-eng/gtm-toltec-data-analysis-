...
class AstropyTab:
    ...
    def plotInPhot(self):
        ...
        if self.inphot==False:
            ...
            plt.figure()
            index_nonoise=np.where(self.astrotab['flux_unc']/self.astrotab['flux_fit']<1.)
            plt.errorbar(self.astrotab['flux_'+self.array+'_input'][index_nonoise],self.astrotab['flux_fit'][index_nonoise],yerr=self.astrotab['flux_unc'][index_nonoise],fmt='.',color='black',ecolor='grey',elinewidth=0.5)
